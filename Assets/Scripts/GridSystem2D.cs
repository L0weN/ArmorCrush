...
public class GridSystem2D<T>
{
    ...
    public static GridSystem2D<T> VerticalGrid(int width, int height, float cellSize, Vector3 origin, bool debug = false) => new GridSystem2D<T>(width, height, cellSize, origin, new VerticalConverter(), debug);

    public static GridSystem2D<T> HorizontalGrid(int width, int height, float cellSize, Vector3 origin, bool debug = false) => new GridSystem2D<T>(width, height, cellSize, origin, new HorizontalConverter(), debug);
    ...
    TextMeshPro CreateWorldText(GameObject parent, string text, Vector3 position, Vector3 direction, int fontSize = 2, Color color = default, TextAlignmentOptions textAnchor = TextAlignmentOptions.Center, int sortingOrder = 0)
    {
    ...
    {
        public override Vector3 GridToWorld(int x, int y, float cellSize, Vector3 origin) => new Vector3(x, y, 0) * cellSize + origin;

        public override Vector3 GridToWorldCenter(int x, int y, float cellSize, Vector3 origin) => new Vector3(x * cellSize + cellSize * 0.5f, y * cellSize + cellSize * 0.5f, 0) + origin;
    ...
    {
        public override Vector3 GridToWorld(int x, int y, float cellSize, Vector3 origin) => new Vector3(x, 0, y) * cellSize + origin;

        public override Vector3 GridToWorldCenter(int x, int y, float cellSize, Vector3 origin) => new Vector3(x * cellSize + cellSize * 0.5f, 0, y * cellSize + cellSize * 0.5f) + origin;
